...
contract('FantomAuction', async function () {
  ...
  describe('Testing `resultAuction()` individually', function () {
    ...
    it('4) cannot result a finished auction that ended with bids below the reserve price as non-owner', async function () {
      await expect(
        fantomauction.connect(hacker).resultAuction(mockerc721.address, FOUR)
      ).to.be.revertedWith('_msgSender() must be auction winner or seller');
    });
    ...
    it('bid successfully placed at `sellerReservePrice` by `winner`', async function () {
      await expect(
        ...
        .to.emit(fantomauction, 'BidPlaced')
        .withArgs(mockerc721.address, TWO, winner.address, sellerReservePrice);
    });
    ...
    it('5) cannot result a successful auction as `other`', async function () {
      await expect(
        fantomauction.connect(other).resultAuction(mockerc721.address, TWO)
      ).to.be.revertedWith('_msgSender() must be auction winner or seller');
    });
    ...
    it('9) cannot result an auction that has already been resulted as `winner`', async function () {
      await expect(
        fantomauction.connect(winner).resultAuction(mockerc721.address, TWO)
      ).to.be.revertedWith('_msgSender() must be auction winner or seller');
    });
    ...
    it('bid successfully placed at `sellerReservePrice` by `bidder`', async function () {
      await expect(
        fantomauction
          .connect(bidder)
          .placeBid(mockerc721.address, THREE, sellerReservePrice)
      )
        .to.emit(fantomauction, 'BidPlaced')
        .withArgs(
          mockerc721.address,
    ...
    it('`bidder` successfully withdrew bid once grace window started', async function () {
      await expect(
        fantomauction.connect(bidder).withdrawBid(mockerc721.address, THREE)
      )
    ...
    it('10) cannot result an auction that ended and had the highest bidder withdraw', async function () {
      await expect(
        fantomauction.connect(seller).resultAuction(mockerc721.address, THREE)
