...
async function callBefore() {
  ...
  await fantomaddressregistry
    .connect(owner)
    .updateAuction(fantomauction.address);
  await fantomaddressregistry
    .connect(owner)
    .updateMarketplace(fantommarketplace.address);
  await fantomaddressregistry
    .connect(owner)
    .updateBundleMarketplace(fantombundlemarketplace.address);
  await fantomaddressregistry
    .connect(owner)
    .updateTokenRegistry(fantomtokenregistry.address);
  await fantomaddressregistry
    .connect(owner)
    .updateRoyaltyRegistry(fantomRoyaltyRegistry.address);
  await fantomaddressregistry
    .connect(owner)
    .updatePriceFeed(fantomPriceFeed.address);
  await fantomauction
    .connect(owner)
    .updateAddressRegistry(fantomaddressregistry.address);
  await fantommarketplace
    .connect(owner)
    .updateAddressRegistry(fantomaddressregistry.address);
  await fantombundlemarketplace
    .connect(owner)
    .updateAddressRegistry(fantomaddressregistry.address);
